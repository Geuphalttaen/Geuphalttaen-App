...
import { ToiletMarker, type MarkerStatus } from '../../components/ToiletMarker';
...
// 화장실 isPublic 상태를 MarkerStatus로 매핑 (실제 상태 정보는 추후 API에서)
...
export default function MapScreen({ navigation }: Props) {
  ...
  const handleDetailPress = useCallback(() => {
    if (selectedToilet) {
      navigation.navigate('Detail', { toiletId: selectedToilet.id });
    }
  }, [selectedToilet, navigation]);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={[styles.searchBar, { top: insets.top + 8 }]}>
        <Text style={styles.searchBarSubtitle}>현재 위치</Text>
