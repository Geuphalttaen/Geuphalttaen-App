...
export default function AuthScreen({ navigation }: Props) {
  ...
  const handleKakaoLogin = useCallback(async () => {
    try {
      setIsLoading(true);
      // @react-native-seoul/kakao-login 활용
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { login: kakaoLogin } = require('@react-native-seoul/kakao-login') as {
  ...
  const handleAppleLogin = useCallback(async () => {
    try {
      ...
      const appleErr = err as { code?: string; message?: string };
      if (appleErr.code === 'ERR_REQUEST_CANCELED') {
        // 사용자가 취소한 경우 무시
        return;
      }
      Alert.alert('로그인 실패', appleErr.message ?? 'Apple 로그인에 실패했습니다');
    } finally {
      setIsLoading(false);
    }
